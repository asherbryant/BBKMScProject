...
def run(args):
    ...
    screened_tfs, tfbs_df_list, tf_count = screen.screen_tfbs(args.tf_directory, tf_list, cell_list, args.sites, args.bases)
    ...
    product_df = analyze.get_product(samples_signal_list, sample_names, screened_tfs, product_file)

    combinations_file = f'{args.out_directory}/{case}_combinations.parquet'
    combinations_df = analyze.get_combinations(samples_signal_list, sample_names, screened_tfs, combinations_file)
...
def main():
    parser=argparse.ArgumentParser(description="Calculate and plot the distances between control and case fragment coverage signals around TFBSs from parquet files.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    
    # TFBS arguments
    parser.add_argument("--tf_dir", help="directory of GTRD TF files", dest="tf_directory", type=str, required=True)
    parser.add_argument("--tfs", help="text file of desired TFs", dest="tf_file", type=str, required=True)
    parser.add_argument("--cells", help="text file of desired cell types", dest="cell_file", type=str, required=True)
    parser.add_argument("--sites", help="number of TFBSs to be aggregated", dest="sites", type=int, default=1000)
    parser.add_argument("--bases", help="number of bases from the center of TFBS", dest="bases", type=int, default=1000)
    ...
    parser.add_argument("--cases", help="cases directories", nargs='+', dest="cases", type=str, required=True)
    parser.add_argument("--cnas", help="ichorCNA file paths", nargs='+', dest="cna_files", type=str, required=True)
